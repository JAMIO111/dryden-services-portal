...
const Header = ({ onOpenMobileNav }) => {
  ...
  return (
    <header className="flex flex-col border-b-1 border-border-color bg-secondary-bg pt-[env(safe-area-inset-top)]">
      <div className="flex justify-between items-center px-6 sm:px-4 gap-2">
        ...
        <div className="flex justify-between items-center py-2 gap-1.5 sm:gap-3">
          <button
            ...
            {isMobileSearchOpen ? (
              <HeaderIcon icon={CgClose} top={7} right={3} />
            ) : (
              <HeaderIcon icon={BsSearch} top={7} right={3} />
            )}
          </button>
          ...
          <div className="hidden mr-2 md:block">
            {profile?.avatar ? (
              <img
                className="rounded-lg border-1 border-border-color w-10 h-10 sm:w-10 sm:h-10 object-cover shrink-0"
                src={profile.avatar}
                alt="Profile Pic"
              />
            ) : (
          ...
          <button
            ...
            <HeaderIcon icon={BsList} top={7} right={3} />
          </button>
        </div>
      </div>
